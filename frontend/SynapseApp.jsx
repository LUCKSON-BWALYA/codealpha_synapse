...
async function decryptMessage(packed, key) {
  if (!subtle || !key) return packed;
  try {
    const b64   = packed.replace(/-/g,"+").replace(/_/g,"/");
    const bytes = Uint8Array.from(atob(b64), c => c.charCodeAt(0));
    const plain = await subtle.decrypt({ name:"AES-GCM", iv: bytes.slice(0,12) }, key, bytes.slice(12).buffer);
    return new TextDecoder().decode(plain);
  } catch { return "[encrypted]"; }
}

async function encryptFile(file, key) {
  if (!subtle || !key) return { ciphertext: await file.arrayBuffer(), iv: new Uint8Array(12) };
  const iv   = window.crypto.getRandomValues(new Uint8Array(12));
